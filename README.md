# System Design Resource Library

Welcome to the comprehensive System Design library. This project contains in-depth explanations, real-time use cases, and code examples for various design patterns and architectural concepts.

## 🏗️ High-Level Design (HLD)
Core architectural components for scalable distributed systems.
- [Load Balancing](./HLD/load-balancing.md) - Distributing traffic across servers.
- [Caching](./HLD/caching.md) - Improving performance with Redis/Memcached.
- [Message Queues](./HLD/message-queues.md) - Asynchronous communication with RabbitMQ/Kafka.
- [Database Sharding](./HLD/database-sharding.md) - Horizontal scaling of databases.
- [Microservices](./HLD/microservices.md) - Building independent, deployable services.
- [Replication & Consistency](./HLD/replication-consistency.md) - CAP theorem and data synchronization.
- [CAP Theorem](./HLD/cap-theorem.md) - The fundamental trade-off in distributed systems.
- [Master-Slave Architecture](./HLD/master-slave-architecture.md) - Scaling reads and handling failovers.
- [CDN (Content Delivery Network)](./HLD/cdn.md) - Serving content from the edge.
- [Circuit Breaker](./HLD/circuit-breaker.md) - Handling cascading failures.
- [Service Discovery](./HLD/service-discovery.md) - Finding dynamic services.
- [Reverse Proxy](./HLD/reverse-proxy.md) - Security, SSL termination, and caching.
- [DNS](./HLD/dns.md) - Domain resolution and routing strategies.

## 🛠️ Low-Level Design (LLD)
Principles and patterns for writing clean, maintainable, and efficient code.
- [SOLID Principles](./LLD/solid-principles.md) - The foundation of OOP design.
- [Strategy Pattern](./LLD/strategy-pattern.md) - Interchangeable algorithms (e.g., Payment methods).
- [Observer Pattern](./LLD/observer-pattern.md) - One-to-many event notification.
- [Factory Pattern](./LLD/factory-pattern.md) - Decoupling object creation.
- [Singleton Pattern](./LLD/singleton-pattern.md) - Ensuring single instances (Thread-safe).
- [Builder Pattern](./LLD/builder-pattern.md) - Constructing complex objects fluently.
- [Decorator Pattern](./LLD/decorator-pattern.md) - Adding behavior dynamically.
- [Command Pattern](./LLD/command-pattern.md) - Encapsulating requests as objects (Undo/Redo).
- [Parking Lot Design](./LLD/parking-lot.md) - A classic LLD interview case study.

## 📝 System Design Interview Questions
In-depth architecture for popular consumer-scale systems.
- [URL Shortener (TinyURL)](./System%20Design%20Interview%20Questions/url-shortener.md)
- [Chat System (WhatsApp)](./System%20Design%20Interview%20Questions/chat-system.md)
- [Video Streaming (YouTube/Netflix)](./System%20Design%20Interview%20Questions/video-streaming.md)
- [API Gateway](./System%20Design%20Interview%20Questions/api-gateway.md)
- [Distributed ID Generator (Snowflake)](./System%20Design%20Interview%20Questions/distributed-id-generator.md)
- [Autocomplete System](./System%20Design%20Interview%20Questions/autocomplete-system.md)
- [News Feed System](./System%20Design%20Interview%20Questions/news-feed-system.md)
- [Web Crawler](./System%20Design%20Interview%20Questions/web-crawler.md)
- [Proximity Service (Yelp/Maps)](./System%20Design%20Interview%20Questions/proximity-service.md)
- [Ticket Booking System (Ticketmaster)](./System%20Design%20Interview%20Questions/ticket-booking-system.md)
- [Distributed Cache (Redis internals)](./System%20Design%20Interview%20Questions/distributed-cache.md)
- [Instagram System Design](./System%20Design%20Interview%20Questions/instagram-design.md)
- [Uber System Design](./System%20Design%20Interview%20Questions/uber-design.md)

## 🎖️ Software Development Best Practices
Must-follow concepts for building high-quality software.
- [DRY vs. WET](./Best%20Practices/dry-wet.md) - Duplication vs. Abstraction.
- [KISS and YAGNI](./Best%20Practices/kiss-yagni.md) - Simplicity and focus.
- [TDD (Test Driven Development)](./Best%20Practices/tdd.md) - Red-Green-Refactor.
- [CI/CD](./Best%20Practices/ci-cd.md) - Pipeline automation.
- [Clean Code](./Best%20Practices/clean-code.md) - Writing for humans.
- [12-Factor App](./Best%20Practices/12-factor-app.md) - Modern SaaS principles.

## 📡 Communication Patterns
Patterns for exchanging data between client and server.
- [Polling](./Communication%20Patterns/polling.md) - Periodic status tracking.
- [WebSockets](./Communication%20Patterns/websockets.md) - Full-duplex real-time communication.
- [Webhooks](./Communication%20Patterns/webhooks.md) - "Push" model events.
- [SSE (Server-Sent Events)](./Communication%20Patterns/sse.md) - Unidirectional server push.
- **[Grokking System Design Building Blocks](./Grokking%20System%20Design/README_GROKKING.md)**: Deep dives into consistent hashing, rate limiting, and more.

---
*Created with ❤️ for System Design enthusiasts.*
